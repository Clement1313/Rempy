...
def time_vs_size(df):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=df["size_val"], y=df["time"], mode="lines+markers", name="time")
    )
    fig.add_trace(
        go.Scatter(
            x=df["size_val"], y=df["time_opti"], mode="lines+markers", name="time_opti"
        )
    )
    fig.update_layout(
...
def bar_chart_time(df):
    last_row = df.tail(1)
    fig = go.Figure(
        [go.Bar(x=["pas opti", "opti"], y=last_row[["time", "time_opti"]].values.flatten())]
    )
    return fig
